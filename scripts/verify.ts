...
async function main() {
  console.log("🔍 Verifying BlockTacToe TicTacToe contract...\n");
  ...
  if (fs.existsSync(deploymentFile)) {
    ...
    if (!contractAddress) {
      ...
      console.error("2. Pass as command line argument: npm run verify:sepolia <address>");
  ...
  // TicTacToe contract has no constructor arguments
  const constructorArgs: any[] = [];

  console.log("🚀 Starting verification process...\n");

  try {
    console.log(`🔍 Verifying TicTacToe contract at ${contractAddress}...`);
    ...
    console.log(`\n✅ TicTacToe contract verified successfully!`);
